...
export function createInitCommand(dependencies: {
  logger: Logger;
}): Command<boolean> {
  function log(msg: string) {
    return dependencies.logger.log("init", msg);
  }

  function addLinesToFile(
    filePath: string,
    linesToAdd: { line: string; ifNotExists: RegExp }[]
  ) {
    let lines: string[] = [];
  ...
  function addPackageJsonSectionFromTemplate(
    sectionName: string,
    templateData: any
  ): void {
    let packageJson = fs.readFileSync("package.json", { encoding: "utf-8" });
    ...
    if (!data[sectionName]) {
      ...
      data[sectionName] = JSON.parse(handlebarsCompile(template)(templateData));
